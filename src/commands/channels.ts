...
export async function addChannel(msg: Message, args: string[]): Promise<void> {
	...
	await msg.channel.createMessage(
		`<#${channel.id}> has successfully been added as a ${status} announcement channel for ${tournament.doc.name}.`
	);
}
...
export async function removeChannel(msg: Message, args: string[]): Promise<void> {
	...
	const status = isPrivate ? "private" : "public";
	await msg.channel.createMessage(
		`<#${channel.id}> has successfully been removed as a ${status} announcement channel for ${tournament.doc.name}.`
	);
}
